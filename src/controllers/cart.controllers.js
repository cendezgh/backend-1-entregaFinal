...
export const create = async (req, res, next) => {
  try {
    ...
    if (!newCart) res.status(404).json({ msg: "Error to create cart!" });
...
export const update = async (req, res, next) => {
  try {
    ...
    if (!cartUpd) res.status(404).json({ msg: "Error to update cart!" });
...
export const remove = async (req, res, next) => {
  try {
    ...
    if (!cartDel) res.status(404).json({ msg: "Error to delete cart!" });
...
export const addProdToCart = async (req, res, next) => {
    try {
      ...
      if (!newProdToUserCart) res.json({ msg: "Error to add product to cart" });
  ...
  export const removeProdToCart = async (req, res, next) => {
    try {
      ...
      if (!delProdToUserCart) res.json({ msg: "Error to remove product to cart" });
  ...
  export const updateProdQuantityToCart = async (req, res, next) => {
    try {
      ...
      if (!updateProdQuantity) res.json({ msg: "Error to update product quantity to cart" });
